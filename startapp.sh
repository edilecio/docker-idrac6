...
if [ ! -f lib/avctKVMIOLinux64.jar ]; then
    ...
    wget -O lib/avctKVMIOLinux64.jar https://${177.125.132.26}:${IDRAC_PORT}/software/avctKVMIOLinux64.jar --no-check-certificate
...
if [ ! -f lib/avctVMLinux64.jar ]; then
    ...
    wget -O lib/avctVMLinux64.jar https://${177.125.132.26}:${5900}/software/avctVMLinux64.jar --no-check-certificate
...
exec java -cp avctKVM.jar -Djava.library.path="./lib" com.avocent.idrac.kvm.Main ip=${177.125.132.26} kmport=5900 vport=5900 user=${root} passwd=${calvin} apcp=1 version=2 vmprivilege=true "helpurl=https://${177.125.132.26}:443/help/contents.html" &
